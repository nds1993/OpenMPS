...
], function(Logger, NDSProps, APIClient, WorkAreaRenderer, ContentHeader, SubContainer, JQGrid, MenuBoxForBaroBox)
{
	var MainMethod =
	{
		...
		onShowComplete: function()
		{
			...
			UCMS.retry(function()
			{
				if( self.$el.find(".resultBody_region .form_box").length == 0
					||
					self._getBoxInstance('menuTree') == null)
				{
					return false;
				}
				
				Renderer.__super__.onShowComplete.call( self );

				self._menu = self._getBoxInstance('menuTree');
				self._menu.on(MenuBoxForBaroBox.EVENT.SELECT_ITEM, function(model)
				{
				...
				// 메뉴 트리 높이 설정
				self._menu.getWidget$Element().css("height", "645px");
	...
	var GridMethod =
	{
		...
		onCreate: function()
		{
			Logger.info("TMCOMT10Renderer.onCreate()");
