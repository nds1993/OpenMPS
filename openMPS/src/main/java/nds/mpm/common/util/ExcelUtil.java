...
import nds.frm.component.file.ConflictSafeBySeq;
import nds.frm.exception.MainException;
import nds.frm.property.IPropertyUtil;
import nds.frm.property.PropertyUtil;
...
public class ExcelUtil {
     ...
     */
    static public String createExcel(String[] column, String[] columnType, ArrayList contentList, String filePath, String sheetName) throws Exception{
        String mkFileName = "";
        try {
    ...
    static public String createExcel(String title,String[] column, String[] columnType,int[] colWidth, ArrayList contentList, String fileNm, String sheetName) throws MainException{
        ...
        try {
    		IPropertyUtil ipropertyUtil = new PropertyUtil();
    		String strUploadPath = ipropertyUtil.getTTFrameProp("FileUpload.dir");
    		
            String filePath = strUploadPath + "/" + fileNm + ".xls";
            // 같은 이름의 파일이 존재하면 다른 이름으로..
            File file = new File( strUploadPath + "/" + FileUtil.getFileNameChop(filePath));
            file = ConflictSafeBySeq.getConflictSafeFile(file);

            // 파일의 경로
            String dirPath = strUploadPath;
            // 생성된 파일의 명칭
    ...
    /**
     * Excel파일을 생성(2개리스트 1개 sheet에 생성, head, detail)
     * @param title String              Title정보
     * @param column String[]           Header정보
     * @param columnType String[]       Column 데이터타입(문자형:C , 숫자형:N , 날짜형:D -- 날짜형 현재 지원안함.)
     * @param headList ArrayList     Excel의 내용의 담은 객체(ArrayList안에 ArrayList를 담고 있는 형태)
     * @param detailList ArrayList     Excel의 내용의 담은 객체(ArrayList안에 ArrayList를 담고 있는 형태)
     * @param fileNm String             Excel 파일의 이름
     * @param sheetName String          Sheet 이름
     * @return String                   생성된 Excel파일 이름
     */
    static public String createExcel(String[] hdcolumn,String[] dtcolumn, String[] hdcolumnType, String[] dtcolumnType, ArrayList headList, ArrayList detailList, String filePath, String sheetName) throws Exception{
        String mkFileName = "";
        try {
            int intSheetNo = 0; //시트번호
            if(sheetName.equals("")){
            ...
            int rownum = 0;
            Label label;
            for (colnum = 0; colnum < hdcolumn.length; colnum++) {
                label = new jxl.write.Label(colnum, 0, hdcolumn[colnum], textTypeHeader);
                sheet.addCell(label);
            }
            ...
            // Content 구성
            if(headList != null){
                for (rownum = 1; rownum <= headList.size(); rownum++) {
                    ArrayList rowList = (ArrayList) headList.get(rownum - 1);
                    for (colnum = 0; colnum < hdcolumn.length; colnum++) {
                        if (hdcolumnType[colnum].equals("N")) {
                            numCell = new jxl.write.Number(colnum, rownum, Double.parseDouble(StringUtil.null2zeroString( (String) rowList.get(colnum))),
                                numberType);
                            sheet.addCell(numCell);
                        }else if (hdcolumnType[colnum].equals("I")) {
                            numCell = new jxl.write.Number(colnum, rownum, Double.parseDouble(StringUtil.null2zeroString( (String) rowList.get(colnum))),
                                intType);
                            sheet.addCell(numCell);
                        }
            ...
            if(detailList != null){
            	...
            	for (colnum = 0; colnum < dtcolumn.length; colnum++) {
                    label = new jxl.write.Label(colnum, rownum, dtcolumn[colnum], textTypeHeader);
                    sheet.addCell(label);
                }
            	
            	rownum++;
            	
                for (rownum = rownum; rownum <= detailList.size(); rownum++) {
                    ArrayList rowList = (ArrayList) detailList.get(rownum - 1);
                    for (colnum = 0; colnum < dtcolumn.length; colnum++) {
                        if (dtcolumnType[colnum].equals("N")) {
                            numCell = new jxl.write.Number(colnum, rownum, Double.parseDouble(StringUtil.null2zeroString( (String) rowList.get(colnum))),
                                numberType);
                            sheet.addCell(numCell);
                        }else if (dtcolumnType[colnum].equals("I")) {
                            numCell = new jxl.write.Number(colnum, rownum, Double.parseDouble(StringUtil.null2zeroString( (String) rowList.get(colnum))),
                                intType);
                            sheet.addCell(numCell);
                        }
    ...
    static public String createExcelEgovMap(
    		List<EgovMap> srcList
    		, String[] column
    		, String[] columnType
    		, String filePath
    		, String fileName
    	...
    	for(EgovMap vo : srcList)
        {
			...
			while (keys.hasNext()) {
				...
				colList.add(vo.get(key_name));
			}
			
			exportList.add(colList);
        }
    	
    	File chkFolder = new File(filePath);
    	
    	if(!chkFolder.exists())
    		chkFolder.mkdir();	
    	
    	return ExcelUtil.createExcel(column, columnType, exportList, filePath + fileName, sheetName);
    }
    
    static public String createExcelEgovMap(
    		List<EgovMap> headList
    		,List<EgovMap> detailList
    		, String[] hdcolumn
    		, String[] dtcolumn
    		, String[] hdcolumnType
    		, String[] dtcolumnType
    	...
    	return createExcel(hdcolumn, dtcolumn, hdcolumnType, dtcolumnType , hdList, dtList, filePath + fileName, sheetName);
    }
    
    /***
     * 
     * List<EgovMap> 를 excel 다운로드 하기위한 유틸 
     * 
     * 	List<EgovMap> srcList
    	String[] column 타이틀 컬럼배열 
    	String[] columnType 데이터 타입배열 
    	String filePath 파일명제외 파일생성 디렉토리, 없으면 생성됨.
    	String fileName 파일명 확장자 포함.
    	
    	return download fileName 
     * */
    static public String createExcelDownloadEgovMapList(
    	...
    	String filePath = jsession.getServletContext().getRealPath("/") + "excel/";
    	String fileName = sheetName + strToday + ".xls";
    	
    	String downloadFile = createExcelEgovMap(srcList, column, columnType, filePath, fileName, sheetName);
    	...
    	if(downloadFile != null){
    		 ...
    		 * 다운로드후 서버파일 삭제.
    		 * */
    		File chkFolder = new File(filePath + fileName);
    ...
    static public String createExcelDownloadEgovMapList(
    	...
    	String downloadFile = createExcelEgovMap(hdList, dtList, hdcolumn, dtcolumn, hdcolumnType, dtcolumnType, filePath, fileName, sheetName);
