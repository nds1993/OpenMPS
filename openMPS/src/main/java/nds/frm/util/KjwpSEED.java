...
//
//		String plainText1 = "222";
//		String phoneEncrypt = KjwpSEED.encrypt(plainText1, key);
//		System.out.println("[ " + plainText1 + " ] Encrypt -> " + phoneEncrypt);
//		String encText1 = "b4gQn7MSnusQ9fUjYPyF9A==";
//		String plainText2 = KjwpSEED.decrypt(encText1, key);
//		System.out.println("[ " + encText1 + " ] Descrypt -> " + plainText2);
//	}
    ...
    private static String encrypt(String str, String key) throws Exception {
        ...
        if (key.length() != 16) return retVal;

		int[] seedKey = KjwpSEED.getSeedRoundKey(key);
		retVal = KjwpSEED.getSeedEncrypt(str, seedKey);
    ...
    private static String decrypt(String str, String key) throws Exception {
        String retVal = "";
		...
		int[] seedKey = KjwpSEED.getSeedRoundKey(key);
		retVal = KjwpSEED.getSeedDecrypt(str, seedKey);
	...
	public static String getSeedDecrypt(String encVal, int[] seedKey) throws Exception{
		...
		for(int i=0; i<encByteList.size(); i++){
			...
			KjwpSEED.SeedDecrypt(encByte, seedKey, tempDecByte);
...
//		System.out.println("암호화된 바이트 시작");
		...
		for(int i=0; i<byteList.size(); i++){
			...
			KjwpSEED.SeedEncrypt(byteVal, seedKey, tempEncVal);
...
//		System.out.println("암호화된 바이트 종료");
	...
	public static int[] getSeedRoundKey(String keyStr) throws Exception{
		...
		KjwpSEED.SeedRoundKey(seedKey, keyStr.getBytes());
...
//			value1[i] = Integer.valueOf(value1[i] ^ value2[i]).byteValue();
			Integer in = new Integer(value1[i] ^ value2[i]);
			value1[i] = in.byteValue();
		}
...
// 	private static Boolean LITTLE 	 = false;
 	...
 	private static boolean ENDIAN	 = KjwpSEED.BIG;				// Java virtual machine uses big endian as a default
