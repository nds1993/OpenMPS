...
/**
 * <p>Title: VOC업무유형을 자동으로 콤보박스를 구성하는 taglib</p>
 * <p>Description: </p>
 ...
 * @version 1.0
 */
public class TypeComboTag  extends BodyTagSupport {
    ...
    public int doEndTag() throws JspException {
        ...
        {           
            ...
            if("1".equals(cdId)){
            	Query = " select LCLS as CD_TP, CNSL_CAT_NM as CD_KNM from CLT_CNSL_TYPE_CD where USE_YN='Y' AND LVL = '"+ cdId +"'";
            }
            else{
                Query = " select MCLS as CD_TP, CNSL_CAT_NM as CD_KNM from CLT_CNSL_TYPE_CD where USE_YN='Y' AND LVL = '"+ cdId +"'";            	
            }
            Query += " order by LCLS, ORD asc ";
...
//                                while(m.hasNext()) if("CD_KNM".equals((String)m.next())) rtnValue = (String)m.getValue();
        ...
        {
            throw new JspException("TypeComboTag", e);          
        }
